...
CSphereMeshIlluminated::CSphereMeshIlluminated(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fRadius, UINT nSlices, UINT nStacks) : CMeshIlluminated(pd3dDevice, pd3dCommandList)
{
	...
	XMFLOAT2* pxmf2Textures= new XMFLOAT2[m_nVertices];
	//CalculateVertexNormals(pxmf2Textures, pxmf3Positions, m_nVertices, NULL, 0);

	CTexturedVertex* pVertices = new CTexturedVertex[m_nVertices];
	for (UINT i = 0; i < m_nVertices; i++) pVertices[i] = CTexturedVertex(pxmf3Positions[i], pxmf2Textures[i]);
	...
	m_pd3dTextureCoord0Buffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, pxmf2Textures, m_nStride * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dTextureCoord0UploadBuffer);
	m_d3dTextureCoord0BufferView.BufferLocation = m_pd3dTextureCoord0Buffer->GetGPUVirtualAddress();
	m_d3dTextureCoord0BufferView.StrideInBytes = m_nStride;
	m_d3dTextureCoord0BufferView.SizeInBytes = m_nStride * m_nVertices;

	if (pxmf3Positions) delete[] pxmf3Positions;
	if (pxmf2Textures) delete[] pxmf2Textures;
...
void CSphereMeshIlluminated::ReleaseUploadBuffers()
{
	CMesh::ReleaseUploadBuffers();
